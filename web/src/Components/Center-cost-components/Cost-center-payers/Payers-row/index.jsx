...
export default function PayerRow(props) {
  ...
  useEffect(() => {
    // New percentage
    let np =
      parseInt(props.paymentPercent) -
      parseInt(props.CCPS[props.index].percentage) +
      parseInt(percent);
  ...
  function deleteCCP() {
    ...
    props.setPaymentPercent(
      parseInt(props.paymentPercent) - parseInt(newCCP.percentage)
    );
  }

  return (
    <div className="mb-5 flex items-center justify-around gap-10 p-2">
      <FormControl variant="standard" sx={{ m: 1, minWidth: 120 }}>
        ...
        >
          {CCs &&
            CCs.map((cc) => {
              return (
                <MenuItem key={cc.idCentroCusto} value={cc.idCentroCusto}>
                  {cc.numeroCentroCusto}
                </MenuItem>
              );
            })}
        </Select>
